# Upgrade Controller Design Document
...
┌─────────────────────────┐
│ Wait for Ready          │
│ Notify completion       │◄──── status: Completed
└─────────────────────────┘
```
...
|----------|------|------|----------|
| **Custom implementation** | Full control, no dependencies | Reinventing the wheel, many edge cases | ❌ Rejected |
| **Flux Image Automation** | Native Flux integration | Only for images, not Helm charts | ❌ Rejected |
| **Renovate with PRs** | Standard workflow, review process | Requires manual PR merge, slower | ❌ Rejected |
| **Renovate dryRun=lookup** | Mature detection, no PRs, JSON output | Extra component (Node.js) | ✅ Selected |

### Git Updates

| Approach | Pros | Cons | Decision |
|----------|------|------|----------|
| **Direct K8s API update** | Simple, fast | Git out of sync, not GitOps | ❌ Rejected |
| **Renovate create PR** | Standard, reviewable | Requires merge step, slower | ❌ Rejected |
| **Direct Git commit** | True GitOps, fast, atomic | Needs Git credentials | ✅ Selected |

### Snapshot Timing

| Approach | Pros | Cons | Decision |
|----------|------|------|----------|
| **Rely on scheduled backups** | Already exists | May not be recent enough | ❌ Rejected |
| **Snapshot after Git commit** | Simpler flow | Race with Flux, might miss | ❌ Rejected |
| **Snapshot before Git commit + suspend Flux** | Guaranteed pre-upgrade state | More complex | ✅ Selected |

### YAML Editing

| Approach | Pros | Cons | Decision |
|----------|------|------|----------|
| **Unmarshal/Marshal (naive)** | Simple | Loses comments, changes formatting | ❌ Rejected |
| **Regex replacement** | No formatting changes | Fragile, no structure awareness | ❌ Considered |
| **yq CLI** | Good preservation | External dependency, slower | ❌ Rejected |
| **kyaml (Kustomize)** | Battle-tested by Flux, Go-native | Rare edge cases | ✅ Selected |

### State Storage

| Approach | Pros | Cons | Decision |
|----------|------|------|----------|
| **PostgreSQL** | Rich queries, transactions | Extra dependency, operational overhead | ❌ Rejected |
| **SQLite** | Simple, embedded | Not HA, backup complexity | ❌ Rejected |
| **Redis** | Fast, pub/sub | Volatile, another component | ❌ Rejected |
| **Kubernetes CRDs** | Native, GitOps-friendly, no extra deps | Limited queries | ✅ Selected |
